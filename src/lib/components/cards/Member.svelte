...
<div class="container member">
      ...
      <div class="info">
        ...
        {#if getEmojis(member)}
          <span
            ><b>Emojis:</b>
            <AwaitHtml
              htmlPromise={new Promise((res) => res(getEmojis(member)))}
              useTwemoji={true}
            /></span
          >
        {/if}
        {#if getBirthday(member)}
          <span
            ><b>Birthday:</b>
            <AwaitHtml
              htmlPromise={new Promise((res) => res(getBirthday(member)))}
              useTwemoji={true}
            /></span
          >
        {/if}
  ...
  <div class="content">
    {#if getDescription(member)}
      <main
        class="desc"
        style={getBanner(member) && getColor(member)
          ? `border-left: 4px solid ${getColor(member)}`
          : ""}
      >
        <div>
          <AwaitHtml
