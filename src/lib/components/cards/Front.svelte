...
<a class="front link" href={`${system? `/s/${member.id}` : `/m/${member.id}`}?${$page.url.searchParams.toString()}`} >
    <div class="card front" style={getColor(member) ? `border-bottom: 4px solid ${getColor(member)};` : ""}>
        {#if getAvatar(member, useProxyAvatar)}
        <img class="avatar" style="width: 100%; aspect-ratio: 1 / 1;" src={getAvatar(member, useProxyAvatar)} alt={`${name}'s avatar`}>
        {/if}
        <div class="desc">
            <h3 class="name"><AwaitHtml htmlPromise={new Promise((res) => res($page.url.searchParams.get("dn") && member.display_name ? member.display_name : name ))} useTwemoji={true} /></h3>
